...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, H * W * channel_2, dtype=dtype, device=device, requires_grad=True))
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(OrderedDict([
      ...
      ('relu2', nn.ReLU()),
      ('flatten', Flatten()),
      ('fc', nn.Linear(channel_2 * 32 * 32, num_classes, bias=True)),
  ]))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(OrderedDict([
        ...
        ('relu1', nn.ReLU()),
        ('conv1', nn.Conv2d(in_channels=Cin, out_channels=Cout, kernel_size=3, stride=stride, padding=1)),
        ('bn2', nn.BatchNorm2d(Cout)),
        ('relu2', nn.ReLU()),
        ('conv2', nn.Conv2d(in_channels=Cout, out_channels=Cout, kernel_size=3, padding=1))
    ]))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
