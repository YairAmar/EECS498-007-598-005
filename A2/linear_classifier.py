...
def svm_loss_vectorized(W, X, y, reg):
    ...
    margin = torch.max(zero, scores - correct_class_score.view(num_train, -1) + 1)
    ...
    mask[torch.arange(num_train), y] = -count
...
def predict_linear_classifier(W, X):
    ...
    """
    scores = torch.mm(X, W)
    y_pred = torch.argmax(scores, dim=1).type(torch.int64)
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-1, 1, 1e1, 1e2, 1e3]
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
    ...
    train_acc = torch.sum(cls.predict(X_train) == y_train) / y_train.shape[0]
    val_acc = torch.sum(cls.predict(X_val) == y_val) / y_val.shape[0]
...
def softmax_loss_naive(W, X, y, reg):
    ...
    for sample in range(batch_size):
        for class_ in range(num_classes):
            f = torch.matmul(X[sample], W)
            f -= f.max()    # numeric stability
            p = f[class_].exp() / f.exp().sum()
            y_class = (y[sample] == class_).type(torch.int)
            dW[:, class_] += (p-y_class) * X[sample, :]
            if class_ == y[sample]:
                loss -= torch.log(p)
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    batch_size = X.shape[0]
    f = torch.matmul(X, W)
    f = f - f.max(dim=1).values.repeat(10, 1).t()  # numeric stability
    p = f.exp() / f.exp().sum(dim=1).repeat(10, 1).t()
    y_dummy = torch.nn.functional.one_hot(y)
    dW = torch.matmul(X.t(), (p - y_dummy)) / batch_size
    log_likelihood = -torch.log(p)
    loss = torch.sum(log_likelihood * y_dummy) / batch_size
    loss += reg * torch.sum(W ** 2)
    dW += 2 * reg * W
    return loss, dW
